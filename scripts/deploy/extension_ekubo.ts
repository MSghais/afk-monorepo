...
import { createExtensionEkubo } from "../utils/extension_ekubo";

dotenv.config();

export const deployExtensionEkubo  = async () => {
  ...
  if (process.env.IS_DEPLOY_CONTRACT == "true") {
    let extensionEkuboContract = await createExtensionEkubo(
      TOKEN_QUOTE_ADDRESS,
      TOKEN_CLASS_HASH,
    ...
    console.log(
        "extensionEkuboAddress address",
      extensionEkuboContract?.contract_address
    );
    if (extensionEkuboContract?.contract_address) {
      launchpad_address = extensionEkuboContract?.contract_address;
