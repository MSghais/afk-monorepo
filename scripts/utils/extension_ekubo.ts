...
dotenv.config();
const PAHT_EXTENSION_EKUBO = path.resolve(
  __dirname,
  "../../onchain/cairo/launchpad/target/dev/afk_launchpad_InternalSwapPool.contract_class.json"
);
const PAHT_EXTENSION_EKUBO_COMPILED = path.resolve(
  __dirname,
  "../../onchain/cairo/launchpad/target/dev/afk_launchpad_InternalSwapPool.compiled_contract_class.json"
);
...
export const createExtensionEkubo = async (
  tokenAddress: string,
  ...
  try {
    ...
    let ExtensionEkuboClassHash = process.env.INTERNAL_SWAP_POOL_CLASS_HASH as string;

    const compiledSierraAAaccount = json.parse(
      fs.readFileSync(PAHT_EXTENSION_EKUBO).toString("ascii")
    );
    const compiledAACasm = json.parse(
      fs.readFileSync(PAHT_EXTENSION_EKUBO_COMPILED).toString("ascii")
    );
    ...
    if (process.env.REDECLARE_CONTRACT == "true") {
      ...
      const declareResponseExtensionEkubo = await account0.declareIfNot({
        contract: compiledSierraAAaccount,
        casm: compiledAACasm,
      });

      if (declareResponseExtensionEkubo?.class_hash) {    
        console.log("Declare deploy", declareResponseExtensionEkubo);
        if (declareResponseExtensionEkubo?.transaction_hash) {
          await provider.waitForTransaction(declareResponseExtensionEkubo?.transaction_hash);
        }
        console.log("DeclareResponse.class_hash", declareResponseExtensionEkubo.class_hash);
      }
      const contractClassHash = declareResponseExtensionEkubo.class_hash;
      ExtensionEkuboClassHash = contractClassHash;
      console.log("ExtensionEkuboClassHash", ExtensionEkuboClassHash);
    ...
    const { transaction_hash, contract_address } =
      await account0.deployContract({
        classHash: ExtensionEkuboClassHash,
        constructorCalldata: [
          accountAddress0,
          accountAddress0,
          tokenAddress,
