...
function PixelHomepage() {
  ...
  const isDesktopOrLaptop = Platform.OS === 'web';
  ...
  const isBigScreen =  windowWidth >= 1824;
  const isTabletOrMobile =  windowWidth >= 1224;
  const isRetina =  windowWidth >= 1224;
  const isPortrait =  windowWidth >= 1224;
  // const isTabletOrMobile = useMediaQuery({ query: '(max-width: 1224px)' });
  // const isPortrait = useMediaQuery({ query: '(orientation: portrait)' });
  // const isRetina = useMediaQuery({ query: '(min-resolution: 2dppx)' });
  // const isMobile = useMediaQuery({ query: '(max-width: 768px)' });
  // const isFooterSplit = useMediaQuery({ query: '(max-width: 52rem)' });
  const isFooterSplit = windowWidth >= 1024;
  // TODO: height checks ?
  ...
  // Websocket
//   const { sendJsonMessage, lastJsonMessage, readyState } = new WebSocket(wsUrl
//   //   {
//   //   share: false,
...
// );
  ...
  useEffect(() => {
    const processMessage = async (message: { messageType: string; color: number; position: any; minter: string; token_id: React.SetStateAction<null>; }) => {
      if (message) {
  ...
  const setPixelSelection = (x: React.SetStateAction<null>, y: React.SetStateAction<null>) => {
