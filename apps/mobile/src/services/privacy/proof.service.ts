...
export const ProofService = {
  async generateTransferProof(proofInputs: TransferProofDto) {
    try {
      const response = await axios.post(
        `${process.env.EXPO_PUBLIC_INDEXER_BACKEND_URL}/api/proof/transfer`,
        proofInputs,
        {
          headers: {
  ...
  async generateApproveProof(props: {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.EXPO_PUBLIC_INDEXER_BACKEND_URL}/api/proof/approve`,
        {
  ...
  async generateTransferFromProof(proofInputs: TransferFromProofDto) {
    try {
      const response = await axios.post(
        `${process.env.EXPO_PUBLIC_INDEXER_BACKEND_URL}/api/proof/transfer-from`,
        proofInputs,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  async generateDepositProof(proofInputs: DepositProofDto) {
    try {
      const response = await axios.post(`${process.env.EXPO_PUBLIC_INDEXER_BACKEND_URL}/api/proof/deposit`, proofInputs, {
        headers: {
          "Content-Type": "application/json",
        },
      });
